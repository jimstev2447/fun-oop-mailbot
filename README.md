...
- Each Village should have buildings and paths properties
- A Village should have an addBuilding method which takes a building and adds it to the village
- A Village should have an AddPath method which takes 2 strings of building addresses and adds an array of the two buildings to its paths

```js
MyVillage.addPath("Jims House", "Mitches House");
MyVillage.paths; // -----> [[{ address:'Jims House', mailbox:[]}, { address:'Mitches House', mailbox:[]}]]
```
...
- Each robot will take a village argument in which they will navigate
- They will have a village, location and a parcels property
- They will have a move method which will:

  Take a location address and update the robots location.

  Not change the location if there is no path from the robots location to that address.
